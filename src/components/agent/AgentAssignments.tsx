  ...
  const fetchAssignments = async () => {
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/enquiries/agent-assignments/",
        {
          headers: {
  ...
  if (!target) return;
    ...
    try {
      ...
      const res = await fetch(
        `http://127.0.0.1:8000/api/masters/templates/${templateId}/structure/`,
        {
          headers: {
            Authorization: `Token ${token}`,
          },
...
//             `http://127.0.0.1:8000/api/responses/inspection/draft/${enq.enquiry_id}/`,
//             {
//                 headers: {
//                     Authorization: `Token ${token}`,
//                 },
...
//     }
      ...
      const statusResp = await fetch(
        `http://127.0.0.1:8000/api/enquiries/${enq.enquiry_id}/change-status/`,
        {
          method: "POST",
          headers: {
...
console.log("Token:", token);
      const res = await fetch(
        "http://127.0.0.1:8000/api/responses/inspection/save-draft/",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmitInspection = async () => {
    ...
    const res = await fetch("http://127.0.0.1:8000/api/responses/inspection/submit/", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Token ${token}`,
    ...
    if (res.ok) {
      for (const [qId, r] of Object.entries(responses)) {
        if (r.mediaUrls?.length) {
          ...
          await fetch("http://127.0.0.1:8000/api/responses/upload-media/", {
            method: "POST",
            headers: {
              Authorization: `Token ${token}`,
            },
      ...
      const resp = await fetch(
        `http://127.0.0.1:8000/api/enquiries/${selected.enquiry_id}/change-status/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
